...
namespace TechTweaking.Bluetooth
{
	...
	public class BluetoothAdapter : MonoBehaviour
	{
		...
		{			
			...
			GameObject BAgameObject = new (BtBridge.unity_game_object_name);
		...
		{
			return FindFirstObjectByType<BluetoothAdapter>() != null;
		}

		//Test if there exist multiple singleton in the scene (used in Awake())
		private static bool is_multiple_singleton_in_scene() 
		{
			return FindObjectsByType<BluetoothAdapter>(FindObjectsInactive.Exclude, FindObjectsSortMode.None).Length > 1;
